...
// A fixed permutation table (one of many valid). Commonly used reference table.
static constexpr std::uint8_t T[256] = {
...
std::uint64_t pearson64(bytespan msg) noexcept {
  std::uint64_t out = 0;
  for (int j = 0; j < 8; ++j) {
    std::uint8_t h = T[(j + static_cast<int>(msg.size())) & 0xFF];
    for (std::byte b : msg) {
      h = T[h ^ static_cast<std::uint8_t>(b)];
    }
    out |= (std::uint64_t)h << (8 * j);
  }
  return out;
}
